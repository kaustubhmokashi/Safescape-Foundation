...
function touchTriggerRuntime_(spreadsheetId, triggerType) {
  var config = getRequiredConfig_(spreadsheetId);
  var updatedConfig = JSON.parse(JSON.stringify(config));

  updatedConfig.trigger = updatedConfig.trigger || {};
  updatedConfig.trigger.lastRunAt = new Date().toISOString();
  updatedConfig.trigger.lastTriggerType = triggerType || '';

  saveConfig_(spreadsheetId, updatedConfig);
  return updatedConfig.trigger.lastRunAt;
}
